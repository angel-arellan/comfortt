...
cy.get('#product_hero_add_to_cart_button-default-product-hero-add-to-cart-button_5_rc > form:nth-child(1) > button').then(($btn) => {
    ...
    // let selectedColor = 'Orchid';  // O el color que hayas seleccionado
    // let selectedSize = 'XL';     // O el tamaño que hayas seleccionado
    
    // cy.get('#cart .cart-item', { timeout: 5000 })
    //   //.should('be.visible')
    //   .and('contain', 'Coordinate Sweatpants')
    //   .and('contain', `Color: ${selectedColor}`)
    //   .and('contain', `Size: ${selectedSize}`);
...
// cy.get('#cart > div > div.zUHyhS7e > div.mhTDcJGU > label > svg', { timeout: 5000 }) // Usa el selector correcto del botón de cerrar
// //.should('be.visible')  // Asegura que el botón sea visible
// .click({ force: true }); // Hace clic en el botón para cerrar el carrito
